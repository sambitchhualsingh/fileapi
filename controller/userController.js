...
const readAll = async (req,res) => {
    try{
        let users = await UserActivation.find({})  // model.find() => to read all values in db collection
        // filter the admin
    ...
    try{
        ...
        let extUser = await User.findById({ _id: id })
            ...
            if(extUser.isActive === true) {
                ...
                await User.findByIdAndUpdate({ _id: id}, { isActive: req.body.isActive })
                res.status(StatusCodes.ACCEPTED).json({ msg: `User is Active successfully`})
            }
    } catch (err) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ msg: err.message })
    }
